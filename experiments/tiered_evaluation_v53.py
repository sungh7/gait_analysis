"""
Tiered gait evaluation pipeline v5.3.

Key updates over v5.2:
1. **Multi-layer pose orientation validation**
2. **Automatic left/right label swap correction**
3. **GT-based cross-matching for swap detection**
4. **Orientation-aware correction logic**
5. Maintains all V5.2 functionality (quality-weighted scaling, cross-leg validation)

Change log from v5.2:
- Added pose orientation validation before analysis
- Added automatic label swap detection using GT cross-matching
- Added _swap_left_right_columns() for label correction
- Enhanced _analyze_temporal_v3() with validation and correction steps
- Added comprehensive validation reporting

Target improvements:
- Right Step Length ICC: 0.14 → 0.50+ (minimum), 0.60+ (target)
- Left/Right outlier ratio: 6:1 → 2:1 or better
- Cross-leg validation pass rate: 56% → 75%+
- Maintain left performance: ICC 0.82
"""
...
class TieredGaitEvaluatorV53(TieredGaitEvaluatorV52):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.auto_apply_symmetric = False
...
def main():
    """Run V5.3 evaluation on full cohort."""

    import json
    from pathlib import Path

    print("="*80)
    print("Tiered Gait Evaluation V5.3")
    print("Pose Validation + Automatic Label Correction")
    print("="*80)
    ...
    output_file = "tiered_evaluation_report_v53.json"
