"""
Tiered gait evaluation pipeline v5.3.2.

Key updates over v5.3.1:
1. **Relaxed pose validation threshold**: 75% → 50%
2. **Relaxed label correction threshold**: 0.9 → 0.95 (10% → 5% improvement)
3. **Conditional symmetric scale application**

Changes from v5.3.1:
- Lowered orientation confidence threshold from 75% to 50%
- More permissive label swap detection (5% improvement threshold)
- Auto-apply symmetric scale when cross-leg validation fails

Expected improvements:
- More subjects eligible for label correction (11/21 → 16-18/21)
- Higher label correction rate (1/21 → 3-5/21)
- Right ICC improvement (0.282 → 0.35-0.45 target)
"""
...
class TieredGaitEvaluatorV53(TieredGaitEvaluatorV52):
    """
    V5.3: Adds pose validation and automatic label correction.
    Inherits quality-weighted scaling from V5.2.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # V5.3.2: Relaxed thresholds for better coverage
        self.orientation_validator = PoseOrientationValidator(confidence_threshold=50.0)  # 75 → 50
        self.auto_correct_labels = True
        self.enable_symmetric_fallback = True
        self.auto_apply_symmetric = True  # False → True (enable conditional application)
...
def main():
    ...
    print("="*80)
    print("Tiered Gait Evaluation V5.3.2")
    print("Relaxed Thresholds (Pose: 50%, Label: 0.95)")
    print("="*80)

    # Initialize evaluator
    evaluator = TieredGaitEvaluatorV53(
