...
![Figure 1](/data/gait/figures/Figure1_Pipeline.png)

**Figure 1. Calibration Pipeline Schematic.**
Flowchart illustrating the sequential processing steps: (1) Raw Video Input, (2) MediaPipe Pose Extraction, (3) Sign Correction, (4) DTW Alignment, (5) Per-Subject Deming Regression, and (6) Final Calibrated Output. This pipeline addresses the specific challenges of sign inversion, temporal misalignment, and amplitude scaling.
...
```
This approach ensures that the specific biomechanical characteristics of each subject are accurately mapped.
...
We validated only flexion/extension (hip) and dorsiflexion/plantarflexion (ankle). Frontal plane (abduction/adduction) and transverse plane (rotation) were not assessed due to single-camera limitations.
...
DTW-based ankle calibration requires subject-specific ground truth, limiting immediate clinical deployment. Development of calibration-free methods is a research priority.

**6. Knee not validated:**
