"""
GAVD Feature Extraction - Full Dataset
Processes all GAVD MediaPipe CSV files with validated pipeline
"""
...
def process_gavd_files():
    """Process ALL GAVD MediaPipe CSV files with our pipeline"""
    ...
    view_path = mediapipe_dir / "right_side"
    mp_files = list(view_path.glob("*.csv"))
    ...
    for i, mp_file in enumerate(mp_files):
        if (i+1) % 10 == 0:
            print(f"Progress: {i+1}/{len(mp_files)} ({100*(i+1)/len(mp_files):.1f}%)")
...
def main():
    print("="*60)
    print("GAVD Feature Extraction - FULL DATASET")
    print("="*60)
    ...
    # Save results
    output_file = "/data/gait/gavd_extracted_features_full.csv"
